...
ServerOP::ServerOP(string json)
{
    ...
    std::cout << m_dbUser << m_dbPwd<<m_dbConnStr<<std::endl;

	// 实例化一个连接oracle数据的对象
	m_occi.connectDB(m_dbUser, m_dbPwd,"127.0.0.1",3306, m_dbConnStr);

	// 实例化共享内存对象
	// 从配置文件中读 key/pathname
	string shmKey = root["ShmKey"].asString();

	// 初始化随机数种子：将 srand 放到构造函数中，避免在每次生成随机密钥时重复初始化
	// 这是一处低风险改动，能改善 `getRandKey` 的随机性使用方式
...
void ServerOP::startServer()
{
	...
	while (1)
	{
		...
		cout << "与客户端连接成功..." << endl;
		// 通信
		// 创建工作线程，工作线程将使用传入的 `this` 指针来访问 ServerOP 的成员
		// 注意：workHard 期望接收 `ServerOP*`，因此传参这里直接使用 `this`。
		pthread_t tid;
...
ServerOP::~ServerOP()
{
	if (m_server)
	{
	// srand(time(NULL)); // Removed to avoid repeated seeding
	}
	delete m_shm;
}
