...
// 注：文件中存在若干调试用的 `cout` 输出，生产环境应替换为项目统一日志系统
// 并避免打印敏感信息（如密钥明文）。下面对每个函数添加了说明与风险提示。
...
{
	// 将共享内存全部置 0，初始化所有节点为空
	// 注意：此操作会重置所有数据，调用前应确保这是期望行为
	if (m_shmAddr != NULL)
	{
...
int SecKeyShm::shmWrite(NodeSecKeyInfo * pNodeInfo)
{
	...
	int i = 0;
	// 重要：必须初始化 tmpNodeInfo 为全 0，之前的未初始化变量会导致 UB
	NodeSecKeyInfo tmpNodeInfo{}; // 空结点（值初始化，全部为 0）
